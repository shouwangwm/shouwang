...
public class ImageSizeConverter {
  ...
  private static Logger log = LoggerFactory.getLogger(ImageSizeConverter.class);
  /**
   * 缩放图像
   * 
  ...
  /**
   * 自由缩放图片，按传入参数缩放
   * @param  srcImageFile 原图片全路径
   * @param  result  缩放后的图像地址
   * @param  width  宽
   * @param  height    高
   */
  public static void scaleFree(String srcImageFile, String result, int width, int height) {
    try {
      BufferedImage src = ImageIO.read(new File(srcImageFile)); // 读入文件
      Image image = src.getScaledInstance(width, height, Image.SCALE_DEFAULT);
      BufferedImage tag = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
      ...
      log.error("ImageSizeConverter scaleFree Exception : " + e.getMessage());
      e.printStackTrace();
    }
  }
  ...
  public static void main(String[] args) {
    ...
    String target = "D:/img/src/2-56.jpg";
    scaleFree(src, target, 56,56);
  }
}
